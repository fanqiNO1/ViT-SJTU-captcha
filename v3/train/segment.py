...
def segment(image: np.ndarray) -> list[np.ndarray]:
    ...
    M = cv2.getRotationMatrix2D((image.shape[1] / 2, image.shape[0] / 2), -0.22, 1)
    ...
    for contour in contours:
        ...
        if cropped_to_w * KUN_RATIO < this_w or this_w / this_h >= KUN_ASPECT_RATIO:
            # Apply SVM classification to split the wide bounding box
            split_points = np.column_stack(
                np.where(thresh[this_y : this_y + this_h, this_x : this_x + this_w] > 0)
            )
            if len(split_points) > 1:
                # Train the SVM model with two classes based on x-coordinates
                model = SVC(kernel="linear", C=1.0, probability=True)
                labels_init = (
                    split_points[:, 1] > np.median(split_points[:, 1])
                ).astype(int)
                ...
                if abs(mean_x_cluster_0 - mean_x_cluster_1) > this_w * 0.4:
                    ...
                    bboxes.append((this_x + split_x, this_y, this_w - split_x, this_h))
                else:
    ...
    # Patch i and j
    # Re-iterate the bboxes, find width < 10 and height < 10 bboxes, and merge it with the closest bbox other than itself
    ...
    # Sort the bounding boxes by x-coordinate again. If 6 bboxes are found, merge two closest bboxes.
    bboxes = sorted(bboxes, key=lambda x: x[0])
...
def main():
    ...
    for file in tqdm(files):
        ...
        if len(segmented_images) != len(labels):
            print(
                f"Warning: Mismatch in file {file}. Found {len(segmented_images)} segments but expected {len(labels)} characters."
            )
